...
# Design Creation 
########################################################################################
# Wenxun: ICC_INIT_DESIGN_INPUT = DDC
...
# to the Milkyway module of Solvnet article #1823238
  ...
  if { ![file exists [which $MW_DESIGN_LIBRARY/lib]] } {
	...
	create_mw_lib \
            -tech $TECH_FILE \
            -bus_naming_style {[%d]} \
	    -mw_reference_library $BACKEND_PATH \
...
if {$ICC_INIT_DESIGN_INPUT == "VERILOG" } {
     ...
     # read_sdc "$DESIGN_REF_DATA_PATH/flow/common_scr/soc_ahblite.sdc"
...
rm_abort_on_errors "init_design input and constraint import"
...
if {$ICC_FLOORPLAN_INPUT == "DEF" && [file exists [which $ICC_IN_DEF_FILE]]} {
    ...
    rm_abort_on_errors "init_design DEF import"
} 
...
# Wenxun : INPUT == "USER_FILE"
if {$ICC_FLOORPLAN_INPUT == "USER_FILE"} {
  if {[file exists [which $ICC_IN_FLOORPLAN_USER_FILE]]} {
    source -echo -verbose $ICC_IN_FLOORPLAN_USER_FILE
  } else {
    echo "RM-Error: floorplan script $ICC_IN_FLOORPLAN_USER_FILE was not found"
    exit 2
  }
}
...
rm_abort_on_errors "init_design_icc"
exit
