...
if {$ICC_TIE_CELL_FLOW} {
  # The DC handoff already contains explicit TIEH/TIEL instances.  Placement
  # optimization otherwise removes some high-fanout hierarchical tie drivers,
  # which later makes write_verilog emit MWDC-229 and undriven implicit wires.
  set rm_existing_tie_cells [all_tieoff_cells]
  if {[sizeof_collection $rm_existing_tie_cells] > 0} {
    set_dont_touch $rm_existing_tie_cells
    set rm_tie_output_pins [get_pins -of_objects $rm_existing_tie_cells \
        -filter "direction == out"]
    set rm_tie_output_nets [get_nets -of_objects $rm_tie_output_pins]
    if {[sizeof_collection $rm_tie_output_nets] > 0} {
      set_dont_touch $rm_tie_output_nets
    }
    echo "RM-Info: Preserving [sizeof_collection $rm_existing_tie_cells] synthesized tie cells and [sizeof_collection $rm_tie_output_nets] tie nets"
  }
...
# pc_set_spec is a live controller-to-IF hierarchical connection in the DC
# handoff.  Physical optimization can prove it redundant for this configuration
# and remove its driver while leaving the hierarchy input as an undriven
# Milkyway net (MWDC-229).  Preserve only this boundary net; its surrounding
# logic remains available for optimization.
set rm_pc_set_spec_pins [get_pins -quiet -hierarchical */pc_set_spec_*]
if {[sizeof_collection $rm_pc_set_spec_pins] > 0} {
  set_dont_touch_network $rm_pc_set_spec_pins -no_propagate
  echo "RM-Info: Preserving pc_set_spec at [sizeof_collection $rm_pc_set_spec_pins] hierarchical pin(s)"
}
set rm_pc_set_spec_drivers [get_cells -quiet \
    x_sub_system/x_core/id_stage_i/controller_i/u116]
if {[sizeof_collection $rm_pc_set_spec_drivers] > 0} {
  set_dont_touch $rm_pc_set_spec_drivers
  echo "RM-Info: Preserving [sizeof_collection $rm_pc_set_spec_drivers] pc_set_spec driver cell(s)"
}
