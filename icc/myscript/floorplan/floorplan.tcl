...
read_pin_pad_physical_constraints [file join $ICC_ROOT myscript floorplan pinpad_const.tcl]
...
source -echo -verbose [file join $ICC_ROOT myscript floorplan vdd_vss_rings_straps.tcl]

# preplace sram
source -echo -verbose [file join $ICC_ROOT myscript floorplan preplace_sram.tcl]
# set blockages, this might be need when you have macro cells and some dead corner that causing clog
source -echo -verbose [file join $ICC_ROOT myscript floorplan blockage.tcl]
# preplace tie down cell
source -echo -verbose [file join $ICC_ROOT myscript floorplan preplace_filltie.tcl]
...
create_route_guide -coordinate [list \
    [list $IO2CORE $IO2CORE] \
    [list [expr {$IO2CORE + $CORE_WIDTH}] [expr {$IO2CORE + $CORE_HEIGHT}]]] \
    -no_signal_layers {M8 M9 AP} -no_preroute_layers {M8 M9 AP} -name RG_N_2Z1A
