...
# A standalone focal run starts from chip_finish.  Re-read optional activity
# data here so focal_opt -power does not depend on how that CEL was produced.
...
if {$ICC_REPORTING_EFFORT == "MED" && $POWER_OPTIMIZATION} {
  redirect -file $REPORTS_DIR_FOCAL_OPT/$ICC_FOCAL_OPT_CEL.power {report_power -nosplit}
}
...
if {$ICC_REPORTING_EFFORT != "OFF" } {
 redirect -file $REPORTS_DIR_FOCAL_OPT/$ICC_FOCAL_OPT_CEL.max.tim {report_timing -nosplit -crosstalk_delta -capacitance -transition_time -input_pins -nets -delay max -max_paths 10 -significant_digits 4}
 redirect -file $REPORTS_DIR_FOCAL_OPT/$ICC_FOCAL_OPT_CEL.min.tim {report_timing -nosplit -crosstalk_delta -capacitance -transition_time -input_pins -nets -delay min -max_paths 10 -significant_digits 4}
}
if {$ICC_REPORTING_EFFORT != "OFF" } {
...
rm_abort_on_errors "focal_opt_icc"
exit
