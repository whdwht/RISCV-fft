...
if {$ICC_FIX_ANTENNA } {
  ...
  ########################################
# Wenxun: We force it to solve antenna violation  
...
# Wenxun: We use our own filler insert script. The command is simple. It also source preroute_stdcells in the end
...
source -echo -verbose insert_core_fillers.tcl

if { [check_error -verbose] != 0} { echo "RM-Error, flagging ..." }

}
  
# Wenxun: CUSTPOM_CHIP_FINISH_POST_SCRIPT == empty
...
if {$ADD_FILLER_CELL } {
  ...
  #          STD CELL FILLERS            #
  ########################################
# Wenxun: We add filler again with the same script. Although I think it doesn't make any changes/ I just add it here since no harm will be made  
##Filler Cells
#    if {$FILLER_CELL != ""} {insert_stdcell_filler -cell_without_metal $FILLER_CELL -connect_to_power $MW_POWER_NET -connect_to_ground $MW_GROUND_NET}
source insert_core_fillers.tcl
...
rm_abort_on_errors "chip_finish_icc"
exit
